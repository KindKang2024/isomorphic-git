import 'package:intl/intl.dart'; // For date formatting, ensure this is in pubspec.yaml

class GitRefStash {
  static String get timezoneOffsetForRefLogEntry {
    final offsetMinutes = DateTime.now().timeZoneOffset.inMinutes;
    final offsetHours = (offsetMinutes / 60).abs().floor();
    final offsetMinutesFormatted = (offsetMinutes % 60).abs().toString().padLeft(2, '0');
    final sign = offsetMinutes > 0 ? '-' : '+'; // Note: Dart's offset is positive for UTC+X, JS is negative
    return '$sign${offsetHours.toString().padLeft(2, '0')}$offsetMinutesFormatted';
  }

  static String createStashReflogEntry(
      Map<String, String> author, String stashCommit, String message) {
  ...
  static List<String> getStashReflogEntry(String reflogString, {bool parsed = false}) {
